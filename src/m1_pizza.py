...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 1:  13 white circles.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(150, 100), 30)
    draw_points_on_circle(window, circle, 13, 'cyan')
    window.close_on_mouse_click()
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(number_of_points):
        circle_border = rg.Circle(points[k], 10)
        circle_border.fill_color = color
        circle_border.attach_to(window)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 1:  200 slices, thin (thickness=1) red lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 200)
    circle.outline_thickness = 3
    pizza(window, circle, 200, 'red', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        lines = rg.Line(points[k], circle.center)
...
def run_test_polygon():
    ...
    # Tests 1 and 2 (on the same window):
    title = 'POLYGON test 3:  10 segments with thin magenta lines.'
    window = rg.RoseWindow(550, 400, title)
    circle = rg.Circle(rg.Point(200, 150), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'magenta', 2)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        if k + 1 == number_of_segments:
            lines = rg.Line(points[k], points[k - number_of_segments + 1])
        else:
            lines = rg.Line(points[k], points[k + 1])
        lines.thickness = thickness
        lines.color = color
        lines.attach_to(window)
...
def run_test_fancy_polygon():
    ...
    # Test 4:
    title = 'FANCY POLYGON test 4:  70 yellow lines on black circle, hops = 23.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 70, 23, 'yellow', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        lines = rg.Line(points[k], points[(k + hops_to_next_point) % number_of_lines])
        lines.thickness = thickness
        lines.arrow = "last"
        lines.color = color
        lines.attach_to(window)
